...
#include "include/orbslam/Sim3Solver.h"
...
Sim3Solver::Sim3Solver(KeyFrame *pKF1,
    ...
    for (int i1 = 0; i1 < mN1; i1++) {
        if (vpMatched12[i1]) {
            ...
            const cv::KeyPoint &kp1 = pKF1->mvKeysSemantic[indexKF1];
            const cv::KeyPoint &kp2 = pKF2->mvKeysSemantic[indexKF2];

            const float sigmaSquare1 = pKF1->mvLevelSigma2[kp1.octave];
            const float sigmaSquare2 = pKF2->mvLevelSigma2[kp2.octave];

            mvnMaxError1.push_back(
              static_cast<unsigned long>(9.210 * sigmaSquare1));
            mvnMaxError2.push_back(
              static_cast<unsigned long>(9.210 * sigmaSquare2));
...
void Sim3Solver::ComputeSim3(cv::Mat &P1, cv::Mat &P2) {
    ...
    cv::eigen(
      N, eval, evec);  // evec[0] is the quaternion of the desired rotation
