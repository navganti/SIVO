...
int ORBmatcher::SearchByProjection(Frame &F,
    ...
    for (size_t iMP = 0; iMP < vpMapPoints.size(); iMP++) {
        ...
        for (auto vit = vIndices.begin(); vit != vIndices.end(); vit++) {
            ...
            if (F.mvRight[idx] > 0) {
                const float er = fabs(pMP->mTrackProjXR - F.mvRight[idx]);
                if (er > r * F.mvScaleFactors[nPredictedLevel])
                    continue;
            }

            const cv::Mat &d = F.mDescriptorsSemantic.row(idx);
            ...
            if (dist < bestDist) {
                ...
                bestLevel = F.mvKeysSemantic[idx].octave;
                bestIdx = idx;
            } else if (dist < bestDist2) {
                bestLevel2 = F.mvKeysSemantic[idx].octave;
                bestDist2 = dist;
            }
        }
...
int ORBmatcher::SearchByBoW(KeyFrame *pKF,
    ...
    vpMapPointMatches = std::vector<MapPoint *>(
      F.numSemanticKeys, static_cast<MapPoint *>(nullptr));
    ...
    while (KFit != KFend && Fit != Fend) {
        if (KFit->first == Fit->first) {
            ...
            for (size_t iKF = 0; iKF < vIndicesKF.size(); iKF++) {
                ...
                const cv::Mat &dKF = pKF->mDescriptorsSemantic.row(realIdxKF);
                ...
                for (size_t iF = 0; iF < vIndicesF.size(); iF++) {
                    ...
                    const cv::Mat &dF = F.mDescriptorsSemantic.row(realIdxF);
                ...
                if (bestDist1 <= TH_LOW) {
                    if (static_cast<float>(bestDist1) <
                        ...
                        const cv::KeyPoint &kp = pKF->mvKeysSemantic[realIdxKF];

                        if (mbCheckOrientation) {
                            float rot =
                              kp.angle - F.mvKeysSemantic[bestIdxF].angle;
...
int ORBmatcher::SearchByProjection(KeyFrame *pKF,
    ...
    for (int iMP = 0, iendMP = vpPoints.size(); iMP < iendMP; iMP++) {
        ...
        for (auto vit = vIndices.begin(); vit != vIndices.end(); vit++) {
            ...
            const int &kpLevel = pKF->mvKeysSemantic[idx].octave;

            if (kpLevel < nPredictedLevel - 1 || kpLevel > nPredictedLevel)
                continue;

            const cv::Mat &dKF = pKF->mDescriptorsSemantic.row(idx);
...
int ORBmatcher::SearchForInitialization(Frame &F1,
                                        ...
                                        int windowSize) {
    int nmatches = 0;
    vnMatches12 = std::vector<int>(F1.mvKeysSemantic.size(), -1);
    ...
    vector<int> vMatchedDistance(F2.mvKeysSemantic.size(), INT_MAX);
    vector<int> vnMatches21(F2.mvKeysSemantic.size(), -1);

    for (size_t i1 = 0, iend1 = F1.mvKeysSemantic.size(); i1 < iend1; i1++) {
        cv::KeyPoint kp1 = F1.mvKeysSemantic[i1];
        ...
        cv::Mat d1 = F1.mDescriptorsSemantic.row(i1);
        ...
        for (auto vit = vIndices2.begin(); vit != vIndices2.end(); vit++) {
            ...
            cv::Mat d2 = F2.mDescriptorsSemantic.row(i2);
        ...
        if (bestDist <= TH_LOW) {
            if (bestDist < (float) bestDist2 * mfNNratio) {
                ...
                if (mbCheckOrientation) {
                    float rot = F1.mvKeysSemantic[i1].angle -
                                F2.mvKeysSemantic[bestIdx2].angle;
                    if (rot < 0.0)
                        rot += 360.0f;
                    int bin = round(rot * factor);
                    if (bin == HISTO_LENGTH)
    ...
    for (size_t i1 = 0, iend1 = vnMatches12.size(); i1 < iend1; i1++)
        if (vnMatches12[i1] >= 0)
            vbPrevMatched[i1] = F2.mvKeysSemantic[vnMatches12[i1]].pt;
...
int ORBmatcher::SearchByBoW(KeyFrame *pKF1,
    ...
    const vector<cv::KeyPoint> &vKeys1 = pKF1->mvKeysSemantic;
    const DBoW2::FeatureVector &vFeatVec1 = pKF1->mFeatVec;
    const vector<MapPoint *> vpMapPoints1 = pKF1->GetMapPointMatches();
    const cv::Mat &Descriptors1 = pKF1->mDescriptorsSemantic;

    const vector<cv::KeyPoint> &vKeys2 = pKF2->mvKeysSemantic;
    const DBoW2::FeatureVector &vFeatVec2 = pKF2->mFeatVec;
    const vector<MapPoint *> vpMapPoints2 = pKF2->GetMapPointMatches();
    const cv::Mat &Descriptors2 = pKF2->mDescriptorsSemantic;
            ...
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                ...
                if (bestDist1 < TH_LOW) {
                    if (static_cast<float>(bestDist1) <
                        ...
                        if (mbCheckOrientation) {
                            float rot =
                              vKeys1[idx1].angle - vKeys2[bestIdx2].angle;
                            if (rot < 0.0)
                                rot += 360.0f;
                            int bin = round(rot * factor);
                            if (bin == HISTO_LENGTH)
...
int ORBmatcher::SearchForTriangulation(
  ...
  const bool bOnlyStereo) {
    ...
    int nmatches = 0;
    std::vector<bool> vbMatched2(pKF2->numSemanticKeys, false);
    std::vector<int> vMatches12(pKF1->numSemanticKeys, -1);
    ...
    while (f1it != f1end && f2it != f2end) {
        if (f1it->first == f2it->first) {
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                ...
                const bool bStereo1 = pKF1->mvRight[idx1] >= 0;

                if (bOnlyStereo)
                    if (!bStereo1)
                        continue;

                const cv::KeyPoint &kp1 = pKF1->mvKeysSemantic[idx1];

                const cv::Mat &d1 = pKF1->mDescriptorsSemantic.row(idx1);
                ...
                for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2;
                    ...
                    const bool bStereo2 = pKF2->mvRight[idx2] >= 0;

                    if (bOnlyStereo)
                        if (!bStereo2)
                            continue;

                    const cv::Mat &d2 = pKF2->mDescriptorsSemantic.row(idx2);
                ...
                if (bestIdx2 >= 0) {
                    const cv::KeyPoint &kp2 = pKF2->mvKeysSemantic[bestIdx2];
...
int ORBmatcher::Fuse(KeyFrame *pKF,
    ...
    for (int i = 0; i < nMPs; i++) {
        ...
        for (vector<size_t>::const_iterator vit = vIndices.begin(),
            ...
            const cv::KeyPoint &kp = pKF->mvKeysSemantic[idx];
            ...
            if (pKF->mvRight[idx] >= 0) {
                ...
                const float &kpr = pKF->mvRight[idx];
...
int ORBmatcher::SearchBySim3(KeyFrame *pKF1,
    ...
    for (int i1 = 0; i1 < N1; i1++) {
        ...
        for (vector<size_t>::const_iterator vit = vIndices.begin(),
            ...
            const cv::KeyPoint &kp = pKF2->mvKeysSemantic[idx];

            if (kp.octave < nPredictedLevel - 1 || kp.octave > nPredictedLevel)
                continue;

            const cv::Mat &dKF = pKF2->mDescriptorsSemantic.row(idx);
    ...
    for (int i2 = 0; i2 < N2; i2++) {
        ...
        for (vector<size_t>::const_iterator vit = vIndices.begin(),
            ...
            const cv::KeyPoint &kp = pKF1->mvKeysSemantic[idx];

            if (kp.octave < nPredictedLevel - 1 || kp.octave > nPredictedLevel)
                continue;

            const cv::Mat &dKF = pKF1->mDescriptorsSemantic.row(idx);
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame,
    ...
    for (int i = 0; i < LastFrame.numSemanticKeys; i++) {
        ...
        if (pMP) {
            if (!LastFrame.mvbOutlier[i]) {
                ...
                int nLastOctave = LastFrame.mvKeysSemantic[i].octave;
                ...
                for (vector<size_t>::const_iterator vit = vIndices2.begin(),
                    ...
                    if (CurrentFrame.mvRight[i2] > 0) {
                        const float ur = u - CurrentFrame.mbf * invzc;
                        const float er = fabs(ur - CurrentFrame.mvRight[i2]);
                        if (er > radius)
                            continue;
                    }

                    const cv::Mat &d = CurrentFrame.mDescriptorsSemantic.row(i2);
                ...
                if (bestDist <= TH_HIGH) {
                    ...
                    if (mbCheckOrientation) {
                        float rot = LastFrame.mvKeysSemantic[i].angle -
                                    CurrentFrame.mvKeysSemantic[bestIdx2].angle;
                        if (rot < 0.0)
                            rot += 360.0f;
                        int bin = round(rot * factor);
                        if (bin == HISTO_LENGTH)
        ...
        if (pMP) {
            if (!pMP->isBad() && !sAlreadyFound.count(pMP)) {
                ...
                if (bestDist <= ORBdist) {
                    ...
                    if (mbCheckOrientation) {
                        float rot = pKF->mvKeysSemantic[i].angle -
                                    CurrentFrame.mvKeysSemantic[bestIdx2].angle;
                        if (rot < 0.0)
                            rot += 360.0f;
                        int bin = round(rot * factor);
